...
## 대시보드 쿼리 설계 Best Practice (MUST)

대시보드 앱에서 Data Gateway로 차트 데이터를 조회할 때, **쿼리 설계 방식**이 성능과 안정성을 결정합니다.

### ❌ 비권장 — 원시 데이터를 가져와 클라이언트에서 집계

```sql
-- 전체 로우를 가져와서 JS/Python으로 group by, sum 처리
...
| 클라이언트 메모리 부담 | 브라우저/서버 OOM 가능 |

### ✅ 권장 — 집계 쿼리를 작성해 결과를 작게 만들기

```sql
-- Athena가 서버에서 집계 → 결과 6행
...
| 원칙 | 설명 |
|------|------|
| **쿼리에서 집계** | `GROUP BY`, `SUM`, `COUNT`, `AVG` 등을 SQL에서 수행하여 결과 크기를 줄임 |
| **결과는 작게** | 차트에 필요한 행 수만 반환 (보통 수십~수백 행이면 충분) |
| **차트 1개 = 쿼리 1개** | 각 위젯에 맞는 집계 쿼리를 개별 작성 |
| **병렬 실행** | 여러 차트 쿼리는 `Promise.all` / `ThreadPoolExecutor`로 동시 실행 (아래 "성능 최적화" 참고) |
| **파티션 조건 필수** | 파티션 테이블은 WHERE에 파티션 컬럼을 반드시 지정하여 스캔 범위 최소화 |
