...
> ℹ️ 여기서 `DB_NAME` 은 **앱이 직접 접속하는 Database 의 이름**입니다. 앱이 DB 드라이버로 직접 연결할 때만 사용합니다.
>
> ⚠️ **Data Gateway 를 통해 실시간 데이터를 쿼리할 때는 이 `DB_NAME` 을 쿼리의 DB 명으로 사용하지 마세요.**
> Data Gateway 쿼리는 `DB.Table` 형태로 대상을 직접 명시하며, 기본(default) DB 가 없습니다.
> (배포된 앱에서 Data Gateway 로 데이터를 조회하는 방법은 `DATA_GATEWAY_VIBE_GUIDE` 문서를 참고하세요.)
...
대시보드 앱에서 Data Gateway를 통해 데이터를 조회할 때, **쿼리 설계 방식**이 성능을 결정합니다.

### ❌ 비권장 — 원시 데이터를 가져와 클라이언트에서 집계

```
SELECT * FROM my_db.orders WHERE dt BETWEEN '2026-01-01' AND '2026-06-30'
→ 수만~수십만 건 반환 → FE/BE에서 group by, sum, avg 계산
```

- Athena 스캔량 과다 → 쿼리 시간 수십 초~수 분
- 대량 데이터 전송 → 504 Gateway Timeout / 413 위험
- 클라이언트 메모리 부담

### ✅ 권장 — 집계 쿼리를 작성해 결과를 작게 만들기

```sql
-- 월별 주문 건수/매출 집계 → 결과 6행
...
- Athena가 서버에서 집계 → 결과가 수 행~수백 행으로 줄어듦
- 전송 데이터 최소화 → 빠른 응답, 504/413 위험 없음
- 차트 라이브러리에 바로 바인딩 가능
...
| 원칙 | 설명 |
|------|------|
| **쿼리에서 집계** | `GROUP BY`, `SUM`, `COUNT`, `AVG` 등을 SQL에서 수행 |
| **결과는 작게** | 차트에 필요한 행 수만 반환 (보통 수십~수백 행) |
| **차트 1개 = 쿼리 1개** | 각 위젯에 맞는 집계 쿼리를 개별 작성 |
| **병렬 실행** | 여러 차트 쿼리는 `Promise.all` / `ThreadPoolExecutor`로 동시 실행 |
